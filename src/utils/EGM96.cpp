...
double
hundu(double p[_coeffs + 1], double sinml[_361 + 1], double cosml[_361 + 1], double gr, double re)
{
    ...
    ac += egm96_data[1][0] + (p[2] * egm96_data[2][0])
          + (p[3] * (egm96_data[3][0] * cosml[1] + egm96_data[3][1] * sinml[1]));
...
void
dscml(double rlon, double sinml[_361 + 1], double cosml[_361 + 1])
{
    double a = sin(rlon);
    double b = cos(rlon);
 ...
 */
void
legfdn(unsigned m, double theta, double rleg[_361 + 1])
{
 ...
 */
void
radgra(double lat, double lon, double *rlat, double *gr, double *re)
{
 ...
 */
double
undulation(double lat, double lon)
{
...
double
egm96_compute_altitude_offset(double lat, double lon)
{

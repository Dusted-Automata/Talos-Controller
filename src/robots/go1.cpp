...
void print_udpState(UT::UDPState& state) {
    printf("\
      TotalCount:    %llu\n\
      SendCount:     %llu\n\
      RecvCount:     %llu\n\
      ...
      RecvCRCError:  %llu\n\
      RecvLoseError: %llu\n\
    ", state.TotalCount, state.SendCount, state.RecvCount, state.SendError, state.FlagError, state.RecvCRCError, state.RecvLoseError);
}
...
void
write_move_cmd(UT::HighCmd& cmd, const Velocity2d &velocity)
{
...
go1_read_state(void* ctx)
{
    ...
    //           << std::endl;
    // std::cout << "QUART: " << imu.quaternion[0] << " | " << imu.quaternion[1] << " |  " << imu.quaternion[2] << " | " << imu.quaternion[3]
    //           << std::endl;

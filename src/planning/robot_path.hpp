...
public:
  ...
  // TODO Change away from iter
  std::optional<size_t> get_stop_number(size_t waypoint_idx) const {
    auto it = std::find(stop_indices.begin(), stop_indices.end(), waypoint_idx);
    if (it != stop_indices.end()) {
      return std::distance(stop_indices.begin(), it);
    }
    return std::nullopt;
  }
  ...
  std::optional<size_t>
  next_stop_number(size_t current_idx, Direction dir) const
  {
      auto wp_idx = next_stop_waypoint(current_idx, dir);
      if (!wp_idx) return std::nullopt;
      return get_stop_number(*wp_idx);
  }
  ...
  segment_length(size_t from_idx) const
  {
      ...
      return cumulative_distance[from_idx + 1] - cumulative_distance[from_idx];
  }
  ...
  std::optional<size_t>
  next_stop_index(size_t current_idx, Direction dir) const
  {
      if (waypoints.empty()) return std::nullopt;

      size_t idx = current_idx;
      size_t checks = 0;
      size_t max_checks = waypoints.size();

      while (checks < max_checks) {
          if (dir == Direction::FORWARD) {
              idx = (idx + 1) % waypoints.size();
              if (!is_loop_ && idx == 0) return std::nullopt; // Reached end
          } else {
              if (idx == 0) {
...
struct Path_Cursor {
    ...
    std::optional<size_t> target_stop_number; // Stop number (0, 1, 2, ...)
    ...
    update_target_stop()
    {
        auto stop_wp = path->next_stop_waypoint(current_waypoint, dir);
        if (stop_wp) {
            target_stop_waypoint = *stop_wp;
            target_stop_number = path->get_stop_number(*stop_wp);
        } else {
            target_stop_waypoint = current_waypoint;
            target_stop_number = std::nullopt;
        }
    }
    ...
    distance_to_next_waypoint() const
    {
        ...
        return seg_length * progress;
    }

    // Distance to the TARGET STOP (ultimate destination)
    f64
    distance_to_target_stop(const Path &path) const
    {
        if (!target_stop_number) return 0.0;
    ...
    Advance_Result
    advance(f64 ds) // TODO: This is faulty
    {
        ...
        if (remaining_distance_to_next_waypoint > 0.3) { // TODO: add padding
            ...
            progress = remaining_distance_to_next_waypoint / seg_length;
            ...
            auto result = advance_waypoint();
            printf(" remaining distance: %f \n", remaining_distance_to_next_waypoint);

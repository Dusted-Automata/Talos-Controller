...
public:
  Quadruped(Trajectory_Controller t_c, MPPI_Controller m_c)
      : Robot(t_c, m_c) { // horizon_steps, num_samples, dt, temperature
  ...
  Pose_State simulate(Pose_State &current_state, Velocity2d &control, double dt) {
    ...
    next_state.orientation.rotate(Eigen::AngleAxisd((control.angular.z() * dt), Vector3d::UnitZ()));
...
int main() {
  ...
  std::vector<Ecef_Coord> waypoints = {{0.0, 0.0, 0.0}, {10.0, 0.0, 0.0}};
  ...
  while (!WindowShouldClose()) {
...

    {
      ...
      showcaseTrajectory(robot.pose_state, robot.mppi_controller.nominal_controls, GetFrameTime(),
                         0.1, ORANGE);
    }
    ...
    Vector2 mouseWorldPos = GetScreenToWorld2D(GetMousePosition(), camera);
    char coordText[500];
    sprintf(coordText, "World: (%.1f, %.1f) Zoom: %.2f Time: %f", mouseWorldPos.x, -mouseWorldPos.y,
            camera.zoom, dt);

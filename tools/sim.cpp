...
// void SimRobot(Ecef_Coord &robotPos, Vector3d arrowVector, std::vector<Ecef_Coord> &waypoints,
//               std::vector<Trajectory_Point> &trajectories, size_t &index)
// {
...
//     {
//         Eigen::Matrix3d rotation = trajectories[index].pose.transformation_matrix.rotation();
//
//         Vector3d translation = trajectories[index].pose.transformation_matrix.translation();
//         movement =
//             rotation * (trajectories[index].velocity.linear +
...
//
//         char robot[500];
//         sprintf(robot, "x: %.2f y: %.2f index: %i  velocity.x: %f velocity.y: %f", robotPos.x(),
//                 robotPos.y(), index, vel.x(), vel.y());
//
...
//     Rectangle robot = {static_cast<float>(robotPos.x()), static_cast<float>(robotPos.y()), 2.0,
//                        1.0};
...
int main()
{
    ...
    std::vector<Ecef_Coord> waypoints = {
        {0.0, 0.0, 0.0}, {100.0, 20.0, 0.0}, {10.0, 10.0, 0.0}, {0.0, 10.0, 0.0}, {0.0, 0.0, 0.0}};
    ...
    MPPI_Controller m_c(50, 500, 0.166, 10.0);
    ...
    // std::vector<Trajectory_Point> trajectories = t_c.generate_trajectory(current, next);
